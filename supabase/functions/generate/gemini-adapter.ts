...
export function parseGeminiGenerateContentResponse(response: unknown, context: { model?: string; operation?: string; configuredMaxOutputTokens?: number } = {}): GeminiNormalizedResult {
  ...
  for (const candidate of candidates) {
    ...
    const usageMetadata = isRecord((response as Record<string, unknown>).usageMetadata) ? (response as Record<string, unknown>).usageMetadata as Record<string, unknown> : undefined
    const common = { model: context.model, operation: context.operation, configuredMaxOutputTokens: context.configuredMaxOutputTokens, usageMetadata, parseStage: 'gemini_envelope', candidateCount: candidates.length, finishReason }
    if (finishReason === 'MAX_TOKENS') throw new GeminiAdapterError('PROVIDER_TRUNCATED_RESPONSE', 'Gemini output was truncated by token limit', common)
    if (BLOCKED_FINISH_REASONS.has(finishReason ?? '')) throw new GeminiAdapterError('PROVIDER_BLOCKED_RESPONSE', `Gemini blocked the response: ${finishReason}`, common)
    ...
    if (text) {
      return { provider: 'google', text, finishReason, diagnostics: { ...common, partCount: parts.length, textPartCount: textParts.length, outputLength: text.length, parseStage: 'model_output' } }
    }
  }
