...
export const RUNTIME_RENDERER_VERSION = "phone-screen-v4" as const;
...
export interface RuntimeBaselineManifest { version: "2.0.0"; generatedAt: string; viewport: typeof CANONICAL_VIEWPORT; rendererVersion: typeof RUNTIME_RENDERER_VERSION; entries: RuntimeBaselineEntry[]; knownFailures: string[]; }
...
export function createEmptyRuntimeBaselineManifest(generatedAt: string): RuntimeBaselineManifest { return { version: "2.0.0", generatedAt, viewport: CANONICAL_VIEWPORT, rendererVersion: RUNTIME_RENDERER_VERSION, entries: [], knownFailures: [] }; }
...
export function validateRuntimeBaselineManifest(manifest: RuntimeBaselineManifest): string[] { const issues: string[] = []; if (manifest.version !== "2.0.0") issues.push("UNSUPPORTED_MANIFEST_VERSION"); if (manifest.viewport.width !== 390 || manifest.viewport.height !== 844) issues.push("NON_CANONICAL_VIEWPORT"); if (manifest.rendererVersion !== RUNTIME_RENDERER_VERSION) issues.push("RENDERER_VERSION_MISMATCH"); const keys = new Set<string>(); for (const entry of manifest.entries) { const key = `${entry.mode}:${entry.archetype}`; if (keys.has(key)) issues.push(`DUPLICATE_ENTRY:${key}`); keys.add(key); if (entry.viewport.width !== 390 || entry.viewport.height !== 844) issues.push(`ENTRY_VIEWPORT_MISMATCH:${key}`); if (!entry.screenshotPath || !entry.boundsPath || !entry.treeSignature || !entry.candidateHash) issues.push(`INCOMPLETE_ENTRY:${key}`); } return issues; }
